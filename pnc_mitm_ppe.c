...
int main(int argc, const char * argv[])
{
	...
	cpid = fork();
	if(cpid == 0)
		execve("patched_mitm_communicators/mitm_initiator.bin", parameters, env);
	else
		initiator_pid = cpid;
	...
	printf("MASTER: Incoming connection...\n");

	/* Tell the initiator to start connecting to victim responder */
